#!/usr/bin/env node
/** Optional Eevee turnaround still from a shipped GLB. */
...
const script = join(root, 'art/skills-jam/blender/turnaround.py');
const passArgs = process.argv.slice(2).length ? process.argv.slice(2) : ['--kit', 'player_jam'];
...
if (!blender) {
  console.error('Blender required for render:turnaround. See art/skills-jam/blender/README.md');
  process.exit(1);
}
