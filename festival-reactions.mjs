import {repairHelperSpawns,reactionGuestKitIds} from './festival-npcs.mjs';
...
export class FestivalReactions {
 constructor(kit,{reduced=false,repaired=false}={}){
  ...
  // Stewards + stage companions come from festival-npcs.mjs (repair_assist / reaction list).
  for(const spawn of repairHelperSpawns())kit.place({id:spawn.id,library:'crowd_kit',asset:spawn.asset,position:spawn.position,rotation:spawn.rotation});
  this.guests=reactionGuestKitIds().map(id=>kit.instances.get(id)).filter(Boolean).map(o=>({o,p:o.position.clone(),rotation:o.rotation.y,bones:Object.fromEntries(['upper_arm_L','upper_arm_R','forearm_L','forearm_R','spine','head'].map(n=>{const b=bone(o,n);return[n,b?{b,q:b.quaternion.clone()}:null];}))}));
