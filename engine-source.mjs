...
import { createAtmosphere, WIND, TIME_PRESETS, DEFAULT_TIME_PRESET } from './atmosphere.mjs';
import { FestivalPlay } from './festival-play.mjs';
import {
  TAG_BEHAVIORS, cycleTags, resolveTagTarget, NpcScheduleRunner,
} from './npc-schedule.mjs';
...
import {
  DESTINATIONS as MAP_DESTINATIONS,
  TOUR_ROUTE as MAP_TOUR_ROUTE,
  MAP_PANEL_IDS,
  destinationIds as mapDestinationIds,
  cloneDestination,
  listDestinations,
} from './festival-map-data.mjs';
...
export class FestivalEngine {
 constructor(canvas){
  ...
  this.pmrem=new THREE.PMREMGenerator(this.renderer);this.windStrength={value:WIND.defaultStrength};this.updateLighting(true);createFestivalSky(this);this.atmosphere=createAtmosphere(this);
 ...
 addWind(material,amount){this.windStrength??={value:WIND.defaultStrength};material.onBeforeCompile=shader=>{shader.uniforms.festivalTime=this.time;shader.uniforms.windStrength=this.windStrength;shader.vertexShader='uniform float festivalTime;uniform float windStrength;\n'+shader.vertexShader;shader.vertexShader=shader.vertexShader.replace('#include <begin_vertex>',`#include <begin_vertex>\nvec3 origin=vec3(0.);\n#ifdef USE_INSTANCING\norigin=instanceMatrix[3].xyz;\n#endif\ntransformed.x+=sin(festivalTime*${WIND.batchFreqX}+origin.z*.43+origin.x*.13)*${amount.toFixed(4)}*${WIND.batchAmpX}*windStrength*max(position.y,0.);\ntransformed.z+=cos(festivalTime*${WIND.batchFreqZ}+origin.x*.31)*${amount.toFixed(4)}*${WIND.batchAmpZ}*windStrength*max(position.y,0.);`);};material.customProgramCacheKey=()=>`festival-wind-v2-${amount}`;}
